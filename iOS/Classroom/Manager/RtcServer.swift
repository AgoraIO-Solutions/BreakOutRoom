...
class RtcServer: NSObject {
    ...
    func enableBeauty() {
        if let rtc = rtcEngine {
            isEnableBeauty = true
            Logger.log(message: "rtc enableBeauty", level: .info)
    ...
    func diableBeauty() {
        if let rtc = rtcEngine {
            ...
            Logger.log(message: "rtc diableBeauty", level: .info)
    ...
    func setClientRole(_ role: AgoraClientRole, _ audienceLatencyLevel: Bool) {
        ...
        Logger.log(message: "rtc setClientRole audienceLatencyLevel: \(_audienceLatencyLevel.rawValue)", level: .info)
        ...
        DispatchQueue.main.sync {
            if let rtc = rtcEngine {
                if (enable) {
                    ...
                    Logger.log(message: "rtc disableVideo", level: .info)
                }
            }
        }
    }
    
    func joinChannel(member: ClassroomMember, channel: String, setting: LocalSetting) -> Observable<Result<Void>> {
        guard let rtc = self.rtcEngine else {
