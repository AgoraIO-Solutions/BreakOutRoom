...
class RoomManager: NSObject {
    ...
    var member: ClassroomMember? = nil
...
extension RoomManager: IRoomManager {
    ...
    func getRooms() -> Observable<Result<Array<ClassroomRoom>>> {
        return ClassroomRoom.getRooms()
    }
    
    func create(room: ClassroomRoom) -> Observable<Result<ClassroomRoom>> {
        if let user = account {
            room.anchor = user
            return ClassroomRoom.create(room: room)
                .map { result in
                    if (result.success) {
                        room.id = result.data!
    ...
    func join(room: ClassroomRoom) -> Observable<Result<ClassroomRoom>> {
        if let user = account {
            if (member == nil) {
                member = ClassroomMember(id: "", isMuted: false, isSelfMuted: false, role: .listener, room: room, streamId: 0, user: user)
            }
            member?.isLocal = true
            ...
            if (self.rtcServer.channel == room.id) {
                ...
                return Observable.just(self.rtcServer.isJoinChannel)
                    ...
                    .concatMap { result -> Observable<Result<ClassroomRoom>> in
                        return result.onSuccess { ClassroomRoom.getRoom(by: room.id) }
                    }
                    .concatMap { result -> Observable<Result<Void>> in
    ...
    func subscribeMembers() -> Observable<Result<Array<ClassroomMember>>> {
        ...
        return Observable.combineLatest(
            ...
            .map { [unowned self] (args) -> Result<Array<ClassroomMember>> in
                let (result, _) = args
                if (result.success) {
