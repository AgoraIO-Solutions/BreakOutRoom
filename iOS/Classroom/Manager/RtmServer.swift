...
class RtmServer: NSObject {
    ...
    private var messageReceivedRelay: BehaviorRelay<ClassroomMessage?> = BehaviorRelay(value: nil)
    ...
    func subscribeMessages(room: String) -> Observable<Result<ClassroomMessage>> {
        if (userId == nil) {
            return Observable.just(Result(success: false, message: "login first!"))
        } else {
            return messageReceivedRelay.filter { message -> Bool in
                return message?.channelId == room
            }.map { message -> Result<ClassroomMessage> in
            ...
            return Single.create { single in
                channel.send(AgoraRtmMessage(text: message)) { code in
                    ...
                    if let userId = self.userId {
                        self.messageReceivedRelay.accept(ClassroomMessage(channelId: room, userId: userId, value: message))
                    }
...
extension RtmServer: AgoraRtmChannelDelegate {
    func channel(_ channel: AgoraRtmChannel, messageReceived message: AgoraRtmMessage, from member: AgoraRtmMember) {
        ...
        if (channel == self.channel) {
            if let id = channelId {
                messageReceivedRelay.accept(ClassroomMessage(channelId: id, userId: member.userId, value: message.text))
            }
        }
    }
}
